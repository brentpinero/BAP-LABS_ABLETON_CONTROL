#!/usr/bin/env python3
"""
🎛️ GEMINI SERUM MISTRAL PIPELINE
Ultra-high quality synthetic data generation for Mistral instruction finetuning
Using Gemini 2.5 Flash with best practices from 2024-2025 research
"""
...
class EnhancedParameterMapper:
    ...
    def _map_parameters_complete(self, raw_params: Dict[str, float]) -> Dict[str, float]:
        """Map ALL parameters - let Gemini determine importance"""
        mapped = {}
...
class GeminiInstructionGenerator:
    """Gemini 2.5 Flash powered instruction generation"""
    ...
    async def generate_instructions(self, context: PresetContext, count: int = 1) -> List[Dict]:
        """Generate diverse instructions using Gemini 2.5 Flash"""
        ...
        try:
            ...
            logger.info(f"Gemini raw response length: {len(content) if content else 0}")
            logger.debug(f"GPT-5 raw response: {content[:500] if content else 'None'}")
            instructions = self._parse_gemini_response(content)
    ...
    def _build_system_prompt(self) -> str:
        """Comprehensive system prompt for Gemini"""
...
class QualityValidator:
    ...
    def __init__(self):
        ...
        self.max_instruction_length = 5000  # Allow very detailed Gemini instructions
...
class SerumGeminiPipeline:
    """Main Gemini 2.5 Flash powered pipeline"""

    def __init__(self, config: Dict):
        self.config = config
        self.parameter_mapper = EnhancedParameterMapper(config["parameter_mapping_file"])
        self.gemini_generator = GeminiInstructionGenerator(
    ...
    async def process_preset(self, preset_data: Dict) -> List[SynthenticExample]:
        """Process single preset with Gemini analysis"""
        ...
        try:
            ...
            # Generate instructions with Gemini
            logger.info(f"Processing preset: {context.name}")
            instructions = await self.gemini_generator.generate_instructions(context)
            logger.info(f"🎯 Gemini generated {len(instructions)} instructions for {context.name}")
    ...
    async def generate_dataset(self, input_file: str, output_file: str, max_presets: Optional[int] = None):
        ...
        logger.info("🎛️ Starting Gemini 2.5 Flash Powered Serum Pipeline...")
        logger.info(f"🤖 Using Gemini 2.5 Flash")
...
def main():
    ...
    if not api_key:
        ...
        logger.error("Add your Google API key to .env file")
        return
    ...
    config = {
        ...
        "input_dataset": "data/gemini_presets_500.json",
        "output_file": "data/serum_gemini_mistral_500_dataset.json",
    ...
    # Create pipeline
    pipeline = SerumGeminiPipeline(config)
