...
# Monitor GPT-5 Pipeline Progress

echo "🎛️  GPT-5 SERUM PIPELINE MONITOR"
echo "=================================="
echo ""
...
BATCHES=$(grep "Completed batch" large_pipeline.log | tail -1)
echo "  $BATCHES"

# Count total accepted examples
ACCEPTED=$(grep -c "✅ Accepted" large_pipeline.log)
echo "  Total accepted examples: $ACCEPTED"

# Count processed presets
PROCESSED=$(grep -c "Processing preset:" large_pipeline.log)
echo "  Presets processed: $PROCESSED"

# Count filtered examples
FILTERED=$(grep -c "❌ Filtered" large_pipeline.log)
...
echo "------------------------------------"
grep "✅ Accepted" large_pipeline.log | tail -5 | sed 's/.*overall=0.97 (/  • /' | sed 's/)$//'

echo ""
echo "⏱️  TIMING:"
echo "----------"
START_TIME=$(head -1 large_pipeline.log | awk '{print $1, $2}')
...
echo "💾 LOG SIZE:"
echo "-----------"
ls -lh large_pipeline.log | awk '{print "  " $5}'
